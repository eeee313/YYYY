...
# ========== ROLE ALL ==========
@bot.command(name='role_all')
@commands.has_permissions(administrator=True)
async def role_all_command(ctx, role: discord.Role):
    """Add a role to every member in the server. Usage: !role_all @role"""
    guild = ctx.guild

    # Sanity check up front so we don't burn through the member list only to fail
    ...
    status_msg = await ctx.send(f"⏳ Adding **{role.name}** to all members... this may take a bit.")
    ...
    for member in guild.members:
        ...
        if role in member.roles:
            ...
            continue
        try:
            await member.add_roles(role, reason=f"!role_all run by {ctx.author}")
            added += 1
    ...
    await status_msg.edit(
        content=f"✅ Done. Added **{role.name}** to {added} member(s). {skipped} already had it. {failed} failed."
    )
